...
const ItemComunidad = ({encuesta}) => {
  ...
  return (
    <Grow in={true}>
      <Card>
        <CardHeader
          title = {<Typography variant='body2' fontWeight='bold' >{encuesta.usuario.email}</Typography>}
          subheader={<FechaFormateada fecha={encuesta.publicado} />}
          avatar={<Avatar variant='rounded' sx={{backgroundColor: colors.blueGrey[400]}}  >{encuesta.usuario.username[0]}</Avatar>}
          sx={{backgroundColor: colors.cyan[100], color: colors.cyan[900]}}
        />
        <Divider />
        <CardContent>
          ...
          <Paper elevation={0} variant='outlined' sx={{p: '5px 15px', mt: '16px', backgroundColor:colors.cyan[50]}}>
        ...
        <CardActions sx={{justifyContent:'center', mt:'0'}}>
          <Button variant='outlined' fullWidth onClick={() => router.push(`/responder/${encuesta.id}`)} >Responder</Button>
        </CardActions>
      </Card>
    </Grow>
  )
...
function FechaFormateada({fecha}) {
  ...
  return (
    <Typography variant='body2' sx={{color: 'black'}} >{date.toLocaleString('es',{month: 'long',
    day: 'numeric',
    year: 'numeric',
    })}</Typography>
  )
